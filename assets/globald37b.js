...
class MenuDrawer extends HTMLElement {
  constructor() {
    ...
    this.addEventListener('mouseover', this.onKeyUp.bind(this));
...
class VariantSelects extends HTMLElement {
  ...
  updateMasterId() {
    ...
    // HC - Adjustments to show/hide Klaviyo message on variant change - 15 December '22
    const soldoutDisclaimer = document.querySelector('.disclaimer');
    if (this.currentVariant.available) {
      console.log('available');
      soldoutDisclaimer.style.display = 'none';
      document.querySelector('.product-form__buttons').classList.remove('product-unavailable');
      document.querySelector('.product-form__buttons').classList.add('product-available');
    } else {
      console.log('not available');
      soldoutDisclaimer.style.display = 'flex';      
      document.querySelector('.product-form__buttons').classList.remove('product-available');
      document.querySelector('.product-form__buttons').classList.add('product-unavailable');
    }  
  }
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    if (disable) {
      ...
      // HC - Displaying backorder message - 03 Aug '23
      if (backorder == true ) {
        addButtonText.textContent = 'backorder';
        document.querySelector('.disclaimers .preorder').style.display = 'flex';
      } else {
        addButtonText.textContent = window.variantStrings.addToCart;
        document.querySelector('.disclaimers .preorder').style.display = 'none';
      }
    }
...
// HC - Functions for currency selector - 08 November '22
function currencyFormSubmit(event) {
  event.target.form.submit();
}

document.querySelectorAll('.currency-selector__dropdown').forEach(function(element) {
